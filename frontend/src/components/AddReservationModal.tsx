...
export default function AddReservationModal({
  ...
  const [floor, setFloor] = useState(
    initialData?.floor || initialFloor || "GROUND",
  );
  const [selectedTableIds, setSelectedTableIds] = useState<string[]>(
    initialData?.tableIds ||
      (initialData?.tableId ? [initialData.tableId] : []),
  );
    ...
    const loadTables = async () => {
      try {
        const response = await api.get(
          `/tables?status=AVAILABLE&floor=${floor}`,
        );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0c1424]/40 backdrop-blur-md p-4">
      <div className="w-full max-w-lg rounded-[40px] bg-white p-10 shadow-2xl animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh] custom-scrollbar">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-black text-[#0c1424]">
              {isEditing ? "Modify Booking" : "Add Reservation"}
            </h2>
            <p className="text-sm font-bold text-slate-400 mt-1">
              {isEditing
                ? "Update reservation details"
                : "Strict details required for booking"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="rounded-full p-3 hover:bg-slate-100 transition-colors"
            >
