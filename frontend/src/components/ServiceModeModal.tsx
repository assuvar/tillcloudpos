...
export default function ServiceModeModal({ onClose }: ServiceModeModalProps) {
  ...
  const [availableTables, setAvailableTables] = useState<
    { id: string; name: string; seats: number; floor: string }[]
  >([]);
  const [selectedTableId, setSelectedTableId] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [visibleServiceModels, setVisibleServiceModels] = useState<
    PosOrderType[]
  >([...ALLOWED_SERVICE_MODELS]);
  ...
  useEffect(() => {
    const loadRestaurant = async () => {
      try {
        ...
        if (
          response.data?.serviceModels &&
          Array.isArray(response.data.serviceModels) &&
          response.data.serviceModels.length > 0
        ) {
          setVisibleServiceModels(
            response.data.serviceModels.filter((m: string) =>
              ALLOWED_SERVICE_MODELS.includes(m as any),
            ),
          );
        }
      } catch {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const detailParam =
        selectedType === "DELIVERY" ? deliveryData.name : inputValue;
      navigate(
        `/pos/order-entry?type=${selectedType}&detail=${encodeURIComponent(detailParam || "")}`,
      );
      onClose();
    } finally {
  ...
  const canConfirm = isDelivery
    ? deliveryData.name.trim().length > 0 &&
      deliveryData.address.trim().length > 0 &&
      deliveryData.phone.trim().length > 0
      ...
      <div className="bg-white w-full max-w-[850px] mx-auto rounded-[40px] shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-400 my-auto">
        <div className="p-8 sm:p-10 px-8 sm:px-10">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-[950] text-[#0c1424] mb-2 tracking-tight">
              New Bill
            </h2>
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">
              Select the order type to proceed
            </p>
          </div>
              ...
              return (
                <div
                  ...
                  className={`relative p-6 rounded-[32px] border-2 transition-all cursor-pointer flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 fill-mode-both ${
                    isActive
                      ? "bg-white border-[#0c1424] shadow-2xl scale-[1.01]"
                      : "bg-slate-50/50 border-slate-100/50 hover:border-slate-200"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div
                      className={`h-12 w-12 rounded-xl flex items-center justify-center ${isActive ? "bg-[#0c1424] text-[#5dc7ec]" : "bg-[#f0f7ff] text-[#5dc7ec]"}`}
                    >
                      {<config.icon size={24} />}
                    </div>
                    {isActive && (
                  ...
                  <div className="space-y-1">
                    <h3 className="text-[20px] font-black text-[#0c1424] leading-none tracking-tight">
                          ...
                          >
                            ...
                            {availableTables.map((t) => (
                              <option key={t.id} value={t.id}>
                                {t.name} - {t.floor.toLowerCase()} floor (
                                {t.seats} seats)
                              </option>
                            ))}
                          </select>
          ...
          <div className="flex items-center justify-between pt-8 border-t border-slate-50">
            ...
            <div className="flex items-center gap-8">
              <div className="flex flex-col text-right">
                ...
                <span className="text-lg font-black text-[#0c1424] tracking-tight">
