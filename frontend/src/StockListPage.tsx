...
function StatCard({
  ...
  return (
    <div
      className={`rounded-[24px] p-8 border shadow-sm flex flex-col gap-4 ${isError ? "bg-rose-50 border-rose-100" : "bg-white border-slate-50"}`}
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-baseline gap-2">
          <span
            className={`text-[36px] font-black tracking-tight ${isError ? "text-rose-600" : "text-[#0c1424]"}`}
          >
            {value}
          </span>
          {subValue && (
            <span
              className={`text-[12px] font-bold ${isError ? "text-rose-400" : "text-emerald-500"}`}
            >
              {subValue}
            </span>
          )}
        ...
        <div
          className={`h-12 w-12 rounded-2xl flex items-center justify-center ${isError ? "bg-rose-100 text-rose-600" : "bg-slate-50 text-slate-400"}`}
        >
          {icon}
        </div>
      </div>
...
export default function StockListPage() {
  ...
  const [adjustingItem, setAdjustingItem] =
    React.useState<IngredientRow | null>(null);
  ...
  const handleDeleteIngredient = async (item: IngredientRow) => {
    if (!item.id || item.id === ":id") {
      setError(
        "Cannot delete ingredient: invalid item ID loaded from the server.",
      );
      return;
    }
  ...
  const lowStockCount = ingredients.filter(
    (ingredient) => ingredient.isLowStock,
  ).length;
    ...
    <div className="flex flex-col gap-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-3 font-medium">
            Manage your ingredient levels and menu link requirements.
          </p>
        </div>
      ...
      <div className="bg-white rounded-[40px] border border-slate-50 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          {error ? (
            <div className="px-10 py-4 text-sm font-semibold text-rose-600">
              {error}
            </div>
          ) : null}
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/20">
                <th className="text-left py-8 px-10 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Item Name
                </th>
                <th className="text-center py-8 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Linked Recipes
                </th>
                <th className="text-center py-8 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Current Quantity
                ...
                <th className="text-left py-8 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Last Updated
                </th>
                <th className="text-right py-8 px-10 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {ingredients.map((item) => (
                ...
                >
                  ...
                  <td className="py-8 px-4 text-center">
                    <span
                      className={`text-[15px] font-black ${item.quantity <= 0 ? "text-rose-500" : item.isLowStock ? "text-amber-500" : "text-[#0c1424]"}`}
                    >
                  ...
                  <td className="py-8 px-4">
                    <div className="flex justify-center">
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-2 w-2 rounded-full ${item.quantity <= 0 ? "bg-rose-500" : item.isLowStock ? "bg-amber-500" : "bg-emerald-500"} shadow-sm`}
                        />
                        <span
                          className={`text-[10px] font-black uppercase tracking-[0.1em] ${item.quantity <= 0 ? "text-rose-500" : item.isLowStock ? "text-amber-500" : "text-emerald-500"}`}
                        >
                          {item.quantity <= 0
                            ? "OUT"
                            : item.isLowStock
              ...
              {!isLoading && ingredients.length === 0 ? (
                <tr>
                  ...
                  >
                    No ingredients found. Add your first ingredient to start
                    tracking stock.
                  </td>
                </tr>
              ) : null}
