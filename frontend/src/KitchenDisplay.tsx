...
export default function KitchenDisplay() {
  ...
  const loadOrders = async () => {
    try {
      ...
      setError(
        loadError?.response?.data?.message || "Failed to load kitchen orders",
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-slate-400 font-medium uppercase text-[10px] tracking-widest">
          ...
          {isLoading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {orders.map((order) => (
                <article
                  key={order.id}
                  className="rounded-3xl border border-slate-700 bg-slate-900 p-5 shadow-xl shadow-black/20"
                >
                  <div className="mb-4 flex items-start justify-between gap-3">
                    <div>
                      <div className="text-[10px] font-black uppercase tracking-[0.22em] text-[#5dc7ec]">
                        KOT #{order.kotNumber}
                      </div>
                  ...
                  <div className="mb-4 flex items-center gap-3 text-xs text-slate-400">
                    <Clock3 size={14} />
                    <span>
                      {new Date(order.sentAt).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
                  ...
                  <div className="space-y-3 border-t border-white/10 pt-4">
                    {order.items.map((item) => (
                      ...
                      >
                        ...
                        <div className="text-right text-sm font-black text-[#5dc7ec]">
