...
export default function StaffManagementPage() {
  ...
  const [pinResetTarget, setPinResetTarget] = useState<StaffRecord | null>(
    null,
  );
  ...
  const confirmViewPin = async () => {
    ...
    try {
      const response = await api.post(
        `/staff/${pinResetTarget.id}/pin/reveal`,
        {
          adminPassword,
        },
      );
              ...
              {isLoading ? (
                ...
                <tr>
                  <td
                    ...
                    className="px-8 py-10 text-center text-[14px] font-semibold text-slate-400"
                  >
                    No staff members yet. Add your first staff member to start
                    assigning roles and PIN access.
                  </td>
                </tr>
              ) : (
                staffRows.map((staff) => (
        ...
        <div className="fixed inset-0 z-[120] flex items-end justify-center bg-slate-900/50 p-2 backdrop-blur-sm sm:items-center sm:p-4">
          <div className="w-full max-w-[calc(100vw-1rem)] rounded-[28px] bg-white p-5 shadow-2xl sm:max-w-[440px] sm:p-8">
            ...
            <p className="mt-2 text-[13px] font-semibold text-slate-500">
              Enter your admin password to view or reset PIN for{" "}
              {pinResetTarget.name}.
            </p>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              >
                {actionLoadingId === pinResetTarget.id
                  ? "Loading..."
                  : "View PIN"}
              </button>
              <button
