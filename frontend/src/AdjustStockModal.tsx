      ...
      <div className="w-full max-w-[calc(100vw-1rem)] max-h-[calc(100vh-1rem)] overflow-y-auto rounded-[24px] bg-white shadow-2xl animate-in zoom-in-95 duration-300 sm:max-w-[540px] sm:rounded-[32px]">
          ...
          <div>
            ...
            <div className="flex flex-col mt-1 gap-0.5">
              <p className="text-[13px] text-slate-400 font-medium">
                Modifying:{" "}
                <span className="text-[#0c1424] font-bold">{item.name}</span>
              </p>
              {mode === "SET_FIXED" && (
                <p className="text-[13px] text-slate-400 font-medium">
                  Available:{" "}
        ...
        <div className="grid grid-cols-1 gap-3 px-4 sm:grid-cols-3 sm:px-8">
          ...
          >
            <PlusCircle
              size={18}
              className={mode === "ADD" ? "text-emerald-500" : ""}
            />
          ...
          >
            <MinusCircle
              size={18}
              className={mode === "REMOVE" ? "text-rose-500" : ""}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Remove
            </span>
          ...
          >
            <Edit3
              size={18}
              className={mode === "SET_FIXED" ? "text-emerald-500" : ""}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Set Fixed
            </span>
        ...
        <div className="space-y-6 p-4 sm:p-8">
          ...
          <div className="flex flex-col gap-3 pt-4 sm:flex-row sm:gap-4">
            ...
            <button
              onClick={() =>
                onUpdate({ mode, quantity, reason, itemId: item.id })
              }
        ...
        <div className="flex items-start gap-3 border-t border-slate-100 bg-slate-50/50 p-4 sm:p-6">
          ...
          <p className="text-[11px] font-medium text-slate-400 leading-relaxed">
            Updating inventory will reflect across all linked menu items and
            trigger auto-order alerts if below threshold.
          </p>
        </div>
      </div>
    </div>
